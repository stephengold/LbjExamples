...
However, those are only 3 of the 18 concrete subclasses of
...
|18. {url-shape}/SphericalSegment.html[`SphericalSegment`]
...
While most collision-shape subtypes are precise, 6 of them are not.
For 5 of them, margin uniformly expands their effective shapes:
...
The safest shapes for dynamic rigid bodies are the 6 _primitives_ listed below.
