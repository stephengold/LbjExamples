...
{url-tutorial}/HelloRigidBody.java[HelloRigidBody] is a SPORT
app that demonstrates the 5 familiar features of rigid bodies.
...
is an app that combines static and dynamic rigid bodies.
Things to notice while running the app:
...
reposition them before each time step.
...
is an app that combines kinematic and dynamic rigid bodies.
Things to notice while running the app:
...
is an app that demonstrates contact response.
Things to notice while running the app:
...
is an app that demonstrates damping.
Things to notice while running the app:
...
update the gravity of each body prior to each simulation step.
{url-tutorial}/HelloNonUniformGravity.java[HelloNonUniformGravity]
is an app that demonstrates this technique.
...
To change the deactivation latency (for all rigid bodies),
use `PhysicsBody.setDeactivationDeadline(newInterval)`.
...
is an app that demonstrates deactivation.
Things to notice while running the app:
...
is an app that demonstrates CCD.
Things to notice while running the app:
